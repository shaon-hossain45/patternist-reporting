...
if ( ! class_exists( 'Patternist_Reporting_Public_Display' ) ) {
   ...
   /**
	...
	public function template_shortcode(  $atts, $content = null, $tag = '' ) {
		// Display data risk ranges
        $risk_label = $this->data['result']['risk_label'] ?? '';
        $risk_score = $this->data['result']['risk_score'] ?? 0;
        $angle = $this->data['angle'] ?? 0;

		$start_raw = $this->data['result']['start'] ?? '';
		$end_raw = $this->data['result']['end'] ?? '';

		// Format today's date as: May 12, 2025
		$today_display = ( new DateTime( 'now', new DateTimeZone( 'UTC' ) ) )->format( 'F d, Y' );

        ob_start(); // Start output buffering
        ?>
		<div class="patternist-reporting">
        <div class="reporting-headline">
		...
		<div class="content-part">
			...
			<?php
				// Format start and end if available
				if ( $start_raw && $end_raw ) {
					$start_dt = DateTime::createFromFormat( 'd-m-Y', $start_raw );
					$end_dt   = DateTime::createFromFormat( 'd-m-Y', $end_raw );

					if ( $start_dt && $end_dt ) {
						$start_display = $start_dt->format( 'F d, Y' ); // May 12, 2025
						$end_display = $end_dt->format( 'F d, Y' );
					}
				?>
				<p>Period We Are Analysing: <span style="font-weight: 500;"><?php echo esc_html( $start_display ); ?> – <?php echo esc_html( $end_display ); ?></span></p>
			<?php }; ?>
			<div class="spacer"></div>
			<h4><img src="https://cdn.jsdelivr.net/joypixels/assets/4.5/png/32/1f50d.png" width="18" height="18" class="icon">About This Window</h4>
			<p>This timeframe is based on historical pattern analysis and reflects general sentiment trends. It is provided for informational purposes only.</p>
			<div class="spacer"></div>
			<h4><img src="https://cdn.jsdelivr.net/joypixels/assets/4.5/png/32/26a0.png" width="18" height="18" class="icon">Note</h4>
    ...
    public function template_data() {
        ...
        if( $retro_data ){
			...
			$angle = $this->get_angle_with_bounds( $result['risk_label'], $result['risk_score'] );
	...
	public function get_angle_with_bounds( $risk_level, $risk_score ) {
		$ranges = [
			'low'             => [ 'min' => -90, 'max' => -48 ],
			'moderate'        => [ 'min' => -45, 'max' => -2 ],
			'high'            => [ 'min' => 1,   'max' => 45 ],
			'extremely-high'  => [ 'min' => 48,  'max' => 90 ],
		];
	
		$risk_level = strtolower(str_replace(' ', '-', $risk_level));
	
		$angle = -90 + ($risk_score * 1.8); // Map 0–100 to -90° to 90°
	
		if ( isset( $ranges[ $risk_level ] ) ) {
			$min = $ranges[ $risk_level ]['min'];
			$max = $ranges[ $risk_level ]['max'];
	
			// Clamp the angle within the risk level bounds
			$angle = max($min, min($angle, $max));
		}
